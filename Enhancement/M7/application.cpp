...
void Application::setupScene()
{
	...
	*/
	...
	//generating textures
	generateTextures(tx_table, "textures/table.jpg");
	generateTextures(tx_plate, "textures/plate.png");
	generateTextures(tx_disk, "textures/disk.jpg");
	generateTextures(tx_yellow, "textures/gold.png");
	generateTextures(tx_metal, "textures/metal.jpg");
	generateTextures(tx_dayquil, "textures/dayquil.jpg");
}
...
void Application::generateTextures(unsigned int& texture, std::string file) {
	...
	unsigned char* data = stbi_load(file.c_str(), &width, &height, &nrChannels, STBI_rgb_alpha); //change how we load the stbi so it supports an alpha channel
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data); //generate the textures with alpha channel
